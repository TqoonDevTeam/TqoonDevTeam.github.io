---
title: "후가공_AMCalc_Sub_1_HAKU_New2"
description: "AMCalc_Sub_1_HAKU_New2"
tags: [개발가이드, AMCalc_Sub_1_HAKU_New2 ]
---

### 후가공_AMCalc_Sub_1_HAKU_New2
...
USE [adprintNewDB]
GO
/****** Object:  StoredProcedure [dbo].[AMCalc_Sub_1]    Script Date: 2020-07-28 오후 3:47:44 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
  ...
  SET @달러전환 = ISNULL((SELECT StringValue FROM @TempCalcData where element_id = 5), 100)
  SET @환율 = ISNULL((SELECT StringValue FROM @TempCalcData where element_id = 6), 800)
  SET @야리통수 = ISNULL((SELECT StringValue FROM @TempCalcData where element_id = 7),2000)
  SET @박지단가 = ISNULL((SELECT StringValue FROM @TempCalcData where element_id = 8),0.2)
  SET @동판단가 = ISNULL((SELECT StringValue FROM @TempCalcData where element_id = 9),250)
  SET @가공비기본 = ISNULL((SELECT StringValue FROM @TempCalcData where element_id = 10),100000)
  SET @세금 = ISNULL((SELECT StringValue FROM @TempCalcData where element_id = 11),1.07)
  SET @박비용 = ISNULL((SELECT StringValue FROM @TempCalcData where element_id = 12),0.5)
